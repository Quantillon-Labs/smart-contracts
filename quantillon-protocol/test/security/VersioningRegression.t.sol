...
contract VersioningRegression is DeploymentSmokeTest {
    ...
    function test_AllCoreContractsExposeVersion() public {
        deployFullProtocol();
        _assertVersioned(address(qeuroToken), "QEUROToken", "1.0.3");
        _assertVersioned(address(qtiToken), "QTIToken", "1.0.1");
        _assertVersioned(address(vault), "QuantillonVault", "1.1.3");
        _assertVersioned(address(userPool), "UserPool", "1.0.1");
        _assertVersioned(address(hedgerPool), "HedgerPool", "1.0.3");
        _assertVersioned(address(stQEURO), "stQEUROToken", "1.0.0");
        _assertVersioned(address(feeCollector), "FeeCollector", "1.0.0");
        _assertVersioned(address(yieldShift), "YieldShift", "1.0.1");
    ...
    function test_VersionIsCallableDirectly() public {
        deployFullProtocol();
        assertEq(vault.version(), "1.1.3", "direct call returns semver");
    }
}
