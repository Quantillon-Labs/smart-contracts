...
| L-8 | YieldShift `mockAaveVault` vestigial state | ⚪ | 3a | CODE | ✅ deprecate-in-place (2026-06-17) — `@custom:deprecated` NatSpec; never read/called (purely vestigial). Storage slot/getter/config-param retained: cannot be removed from a live proxy without breaking storage-layout/ABI gates. |
...
| L-5 | UserPool `deposit` redundant loops + unused `totalQeuroToMint` | 🟡 | 4 | CODE | ✅ partial (dead `totalQeuroToMint` removed). Full loop-consolidation **WON'T-FIX** (2026-06-17): the separate passes implement CEI (state-update before external `mintQEURO`); merging would weaken reentrancy safety for negligible gas on bounded arrays (`MAX_BATCH_SIZE=100`). |
